...
public class StreamContext {
     ...
     * with the 4-byte 0x00000001 start code so that downstream FFmpeg / WebCodecs
     * pipelines can decode without out-of-band parameter sets.
     * <p>
    ...
    private byte[] getHvccAnnexBExtradata(BytePointer extradata, int size) {
        ...
        // If already Annex B (start-code prefixed), pass straight through.
        if (data[0] == 0x00 && data[1] == 0x00 && (data[2] == 0x01 || (data[2] == 0x00 && data[3] == 0x01))) {
            return data;
        }
        ...
        try {
            ...
            for (int i = 0; i < numOfArrays; i++) {
                ...
                // First byte: array_completeness (1) | reserved (1) | NAL_unit_type (6).
                // We don't need the NAL type here — the type is also embedded in the
                // first byte of each NAL payload — so just skip it.
                pos++;
